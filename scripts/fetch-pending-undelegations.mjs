...
async function fetchValidators() {
  ...
  if (nextKey) console.warn(`⚠️ Validator pagination truncated — ${validators.length} fetched, more exist`);
...
async function fetchUnbondingForValidator(valoperAddr) {
  ...
  if (nextKey) console.warn(`⚠️ Unbonding pagination truncated for ${valoperAddr.slice(0,24)}`);
  return entries;
}
...
async function main() {
  ...
  const whalePending = allEntries
    ...
    .map(e => ({
      ...
      timestamp: new Date(e.completion_time).toISOString(),  // normalized ISO 8601
      validator: e.validator,
